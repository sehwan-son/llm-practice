...
`run_analysis.sh`는 기본으로 `--device cuda:1,auto`를 사용합니다. 즉 GPU 1을 먼저 시도하고, unavailable이면 `auto`로 fallback합니다. 다른 장치를 쓰려면 환경변수나 CLI flag로 덮어씁니다.
...
bash experiments/triattention_qk_distribution/run_analysis.sh --device cuda:2,auto
```
